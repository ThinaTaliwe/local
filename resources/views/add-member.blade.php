<x-app-layout>
  ...
  <x-sidenav>
    <li class="nav-item">
      <a class="nav-link text-white " href="/memberships">
        ...
        <span class="nav-link-text text-sm ms-1">See all memberships</span>
      </a>
    </li>
  ...
  <main class="main-content position-relative mt-7  h-100 mb-10 border-radius-lg ">
    ...
    <div class="row mt-5 mb-4">
      ...
      <button class="multisteps-form__progress-btn" type="button" title="Membership Type">Membership Type</button>
      </div>
      </div>
      </div>
      <div class="card-body">
      ...
      <div class="col-12 col-sm-6">
        <div  class="input-group input-group-outline  @error('Name') is-invalid focused is-focused  @enderror mt-3 mb-0" >
          ...
          <input type="text" class="multisteps-form__input form-control" name="Name" id="Name"  value="{{ old('Name') }}" placeholder="Name">
        </div>
        @error('Name')
      ...
      <h5 class="font-weight-bolder mb-0">Membership Type</h5>
      <p class="mb-0 text-sm">Select Membership Type</p>
      <div class="multisteps-form__content mt-3">
      <div class="row">
      <div class="col-12 mt-3">
        <div class="dropdown">
          <select  id="memtype" name="memtype" class="btn bg-gradient-secondary shadow-dark dropdown-toggle w-100 my-4 @error('memtype') is-invalid @enderror" style="height: 80px;"  aria-label="Select Membership Type">
            <option selected value="0" disabled>--Select Membership Type-- </option>
            
            @foreach ($memtypes as $memtype)
            {{-- <option value="{{ $memtype->id }}">{{ $memtype->id }}. {{ $memtype->name }} - {{ $memtype->description }} - R{{ round($memtype->membership_fee,2) }}</option> --}}
            @if (old('memtype') == $memtype->id)
            <option value="{{ $memtype->id }}" selected>{{ $memtype->id }}. {{ $memtype->name }} - {{ $memtype->description }} - R{{ round($memtype->membership_fee,2) }}</option>
            @else
                  <option value="{{ $memtype->id }}">{{ $memtype->id }}. {{ $memtype->name }} - {{ $memtype->description }} - R{{ round($memtype->membership_fee,2) }}</option>
            @endif
            @endforeach
      ...
      <div class="button-row d-flex mt-4">
      <button class="btn btn-outline-dark mb-0 js-btn-prev" type="button" title="Prev">Prev</button>
      <button class="btn bg-gradient-success ms-auto mb-0" type="submit" title="Add">Add</button>
      </div>
      </div>
      </div>
